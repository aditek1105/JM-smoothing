...
def normal_smoothing_pos(positive_words_dict, total_positive_count, total_words, total_negative_count):
    lambda_value = 0.5
    pos_words_prob = {}
    for i in positive_words_dict.keys():
        pos_words_prob[i] = ((1 - lambda_value) * (positive_words_dict[i] / total_positive_count)) + (
                lambda_value * (total_words[i] / (total_negative_count + total_positive_count)))
    return pos_words_prob


def normal_smoothing_neg(negative_words_dict, total_negative_count, total_words, total_positive_count):
    lambda_value = 0.5
    neg_words_prob = {}
    for i in negative_words_dict.keys():
        neg_words_prob[i] = ((1 - lambda_value) * (negative_words_dict[i] / total_negative_count)) + (
                    lambda_value * (total_words[i] / (total_negative_count + total_positive_count)))
...
def main(inputdir, output):
    ...
    neg_words_prob = normal_smoothing_neg(negative_words_dict, total_negative_count, total_words,
                                          total_positive_count)
    pos_words_prob = normal_smoothing_pos(positive_words_dict, total_positive_count, total_words,
